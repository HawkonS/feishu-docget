...
- 高级格式清洗：支持正文段落样式、标题对齐、图片样式、表格布局、页边距、标题编号清理等。
...
│   ├── static/vendor/           # 本地前端第三方库，避免运行时引用 CDN
│   └── web/templates/
│       ├── index.html           # 前台下载页面
│       ├── dashboard.html       # 管理后台
│       └── login.html           # 管理后台登录页
...
- 基础设置：封面、标题、页边距、文档信息。
- 文本设置：文本规则、标题对齐、列表样式、正文字号、段落间距。
- 图片设置：图片尺寸、图片对齐、图片边框（颜色和粗细）及图片缩小比例。
...
图片所在段落也会使用独立的 `图片` 预设样式，因此修改正文样式不会影响图片段落的段前段后距、行距等设置。模板已有同名样式时同样优先复用，非段落类型则使用 `图片（导出）` 等后缀名称。
...
只要已配置 `feishu.app_id` 和 `feishu.app_secret`，管理后台登录页就会显示飞书登录入口；前台普通用户登录仍由 `login.enabled` 控制。
用户角色分为系统管理员和运营管理员：系统管理员可管理配置、模板、用户角色及系统操作，
运营管理员负责日常后台查看和运营操作，不能管理系统管理员。系统管理员账号不能被禁用，且系统管理员数量不设上限。
通过飞书登录的系统管理员仍显示其飞书姓名。
